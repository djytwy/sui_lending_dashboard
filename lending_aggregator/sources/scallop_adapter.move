/// Scallop adapter boundary. See `adapter_notes.move` before replacing the
/// escrow fallback with real Scallop mainnet calls.
module lending_aggregator::scallop_adapter;
...
entry fun deposit<T>(
    config: &Config,
    vault: &mut Vault<protocols::Scallop, T>,
    coin: Coin<T>,
    ctx: &mut TxContext,
) {
...
entry fun withdraw<T>(
    ...
    position: core::Position<protocols::Scallop, T>,
    ctx: &mut TxContext,
) {
